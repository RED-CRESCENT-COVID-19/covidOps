			...
			Http.post('auth/phone', { phone: phone })
				.then((response) => {
					...
					if (response.status === 204) {
						...
						Alert.alert('Info', message, [{ text: 'OK', onPress: () => console.log('OK Pressed') }], { cancelable: false });
					}
				})
				.catch((err) => {
					this.setState({ isLoading: false });
	...
	render() {
		...
		return (
			<View style={Styles.container}>
				...
				<Text style={[Styles.topParagraph, style]}>{I18n.t(`Paragarphs.PHONEVERIFICATION`)}</Text>
