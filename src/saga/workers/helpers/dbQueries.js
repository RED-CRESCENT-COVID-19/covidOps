...
export const getHouse = () => {
	return new Promise(
		(resolve, reject) => {
			db.transaction((tx) => {
				tx.executeSql(`select * from house;`, null, (_, { rows: { _array } }) => {
...
export const addHouse = (params, token) => {
	...
	return new Promise((resolve) => {
		db.transaction(
			(tx) => {
				tx.executeSql('insert into house (id,token, address,lat,lng,is_contacted,createdAt) values (?, ?, ?, ?, ?, ?,?)', [
					id,
					token,
					address,
					lat,
...
export const getUsersForHouseId = (houseId) => {
	...
	return new Promise((resolve) => {
		db.transaction(
			(tx) => {
				tx.executeSql(`select * from user where house_id = '${houseId}'`, null, (_, { rows: { _array } }) => resolve(_array));
			},
